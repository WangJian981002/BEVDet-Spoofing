...
def inference():
    ...
    vis_imgs = []
    for i in range(6):
        img = sigle_data_dict['img_inputs'][0][0][i]
...
def inference_XGD():
    ...
    #print(result)
    predict_score = result[0]['pts_bbox']['scores_3d']
    mask = predict_score>0.25
    predicted_box = result[0]['pts_bbox']['boxes_3d']
    predicted_box.corners[mask].numpy().tofile('predict_anno.bin')


    img = sigle_data_dict['img_inputs'][0][0][0]
    img = mmlabDeNormalize(img)  # rgb (H,W,3)
    cv2.cvtColor(img, cv2.COLOR_RGB2BGR, img)

    cam2lidar = torch.eye(4)
    cam2lidar[:3, :3] = sigle_data_dict['img_inputs'][0][1][0]
    cam2lidar[:3, 3] = sigle_data_dict['img_inputs'][0][2][0]
    cam2img = torch.eye(4)
    cam2img[:3, :3] = sigle_data_dict['img_inputs'][0][3][0]
    lidar2img = cam2img.matmul(torch.inverse(cam2lidar))
    post_rot, post_tran = sigle_data_dict['img_inputs'][0][4][0], sigle_data_dict['img_inputs'][0][5][0]
    ...
    for obj_i in range(len(gt_coner)):
        ...
        for line_i in connect:
            ...
            if is_in_img(p1, p2, img.shape[0], img.shape[1]):
                cv2.line(img, (int(p1[0]), int(p1[1])), (int(p2[0]), int(p2[1])), (64, 128, 255), thickness=2)
    cv2.imwrite('demo_pred.png', img)

def inference_XGD_cons(target_scene = 'scene01'):
    args = parse_args()
    ...
    data_infos = train_dataset.data_infos
    for _ in range(len(data_infos)):
        if data_infos[_]['scene'] == target_scene:
            start_ind = _
    ...
    name_i = 0
    while True:
        if start_ind < len(train_dataset) and data_infos[start_ind]['scene'] == target_scene:
            sigle_data_dict = train_dataset[start_ind]
            data = collate([sigle_data_dict], samples_per_gpu=1)
            if next(model.parameters()).is_cuda:
                data = scatter(data, [device.index])[0]
            with torch.no_grad():
            ...
            mask = predict_score > 0.1
            predicted_box = result[0]['pts_bbox']['boxes_3d']
            predicted_box.tensor[:,2] = predicted_box.tensor[:,2]-0.4 #修正激光雷达高度补正
            ...
            for obj_i in range(len(gt_coner)):
                ...
                for line_i in connect:
                    ...
                    if is_in_img(p1, p2, img.shape[0], img.shape[1]):
                        cv2.line(img, (int(p1[0]), int(p1[1])), (int(p2[0]), int(p2[1])), (64, 128, 255), thickness=2)
            cv2.imwrite('seq_out/'+str(name_i)+'.jpg', img)
...
if __name__ == '__main__':
    #inference()
    #inference_XGD()#XGD和KITTI数据集都可以用这个函数推理
    inference_XGD_cons('scene208')
