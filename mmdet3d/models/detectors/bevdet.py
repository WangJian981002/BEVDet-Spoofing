...
#for response
@DETECTORS.register_module()
class BEVDet_backbone(CenterPoint):
    r"""BEVDet paradigm for multi-camera 3D object detection.
    ...
    def __init__(self,  **kwargs):
        super(BEVDet_backbone, self).__init__(**kwargs)
    ...
    def forward_train(self,
        ...
        Returns:
            dict: Losses of different branches.
        """
        img_feats= self.extract_feat(points, img=img_inputs, img_metas=img_metas, **kwargs)
...
class SegNet(nn.Module):
    def __init__(self, H=256, W=704):
        ...
        self.classification_layer = nn.Conv2d(256, 1, 1, 1, 0)
        self.size = (H, W)
...
class BEVDet_Seg(CenterPoint):
    ...
    def __init__(self, img_view_transformer, img_bev_encoder_backbone,
                 img_bev_encoder_neck, **kwargs):
        super(BEVDet_Seg, self).__init__(**kwargs)
        self.img_view_transformer = builder.build_neck(img_view_transformer)
        self.img_bev_encoder_backbone = \
            builder.build_backbone(img_bev_encoder_backbone)
        self.img_bev_encoder_neck = builder.build_neck(img_bev_encoder_neck)

        self.Patch_seg_Net = SegNet(H=256, W=704)
        self.Patch_seg_Net.load_state_dict(torch.load('Spoofing3D/work_dir_disappear/Seg-disappear3/SegNet_1.pth'))
        self.Patch_seg_Net.eval()
    ...
    def forward_test(self,
        ...
        Args:
            ...
            img (list[torch.Tensor], optional): the outer
                ...
                all images in the batch. Defaults to None.
        """
        imgs = img_inputs[0][0]  # (1,6,3,H,W)
        msks = self.Patch_seg_Net(imgs.squeeze(0)).unsqueeze(0)  # (1,6,1,H,W)

        mean = np.array([[123.675, 116.28, 103.53]], dtype=np.float32) / 255  # bgr下
        std = np.array([[58.395, 57.12, 57.375]], dtype=np.float32) / 255
        pad = (torch.zeros(1, 3) - torch.from_numpy(mean)) / torch.from_numpy(std)
        pad = pad.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).repeat(1, 6, 1, imgs.size()[-2],imgs.size()[-1]).cuda()  # (1,6,3,H,W)

        imgs = torch.where(msks.repeat(1, 1, 3, 1, 1) > 0.5, pad, imgs)
        img_inputs[0][0] = imgs
