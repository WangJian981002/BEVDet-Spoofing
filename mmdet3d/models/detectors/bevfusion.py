...
@DETECTORS.register_module()
class BEVFusionv2_tf(MVXTwoStageDetector):

    def __init__(self, img_view_transformer, fusion_neck, **kwargs):
        super(BEVFusionv2_tf, self).__init__(**kwargs)
        self.img_view_transformer = builder.build_neck(img_view_transformer)
        self.fusion_layer = builder.build_neck(fusion_neck)
    ...
    def extract_feat(self, points, img, img_metas, **kwargs):
        ...
        img_feats, depth = self.extract_img_feat(img, img_metas, **kwargs)
        pts_feats = self.extract_pts_feat(points)
        fused_feats = self.fusion_layer([img_feats,pts_feats[0]])
        return [fused_feats]
...
@DETECTORS.register_module()
class BEVDet_tf_woBEVEncoder(MVXTwoStageDetector):

    def __init__(self, img_view_transformer, **kwargs):
        super(BEVDet_tf_woBEVEncoder, self).__init__(**kwargs)
        self.img_view_transformer = builder.build_neck(img_view_transformer)
    ...
    def forward_train(self,
        ...
        feats = self.extract_feat(
            img=img_inputs, img_metas=img_metas, **kwargs)


        losses = dict()
        losses_pts = self.forward_pts_train(feats, None, gt_bboxes_3d,
                                            gt_labels_3d, img_metas,
                                            gt_bboxes_ignore)
        losses.update(losses_pts)
    ...
    def forward_test(self,
        ...
        if not isinstance(img_inputs[0][0], list):
            img_inputs = [img_inputs] if img_inputs is None else img_inputs
            points = [points] if points is None else points
            return self.simple_test( img_metas[0], img_inputs[0],
                                    **kwargs)
        else:
            return self.aug_test(None, img_metas[0], img_inputs[0], **kwargs)
...
@DETECTORS.register_module()
class BEVDet_tf(MVXTwoStageDetector):

    def __init__(self, img_view_transformer, **kwargs):
        super(BEVDet_tf, self).__init__(**kwargs)
        self.img_view_transformer = builder.build_neck(img_view_transformer)
