...
class Dual_DCA(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_cfg=dict(type='BN', eps=1e-3, momentum=0.01),
                 init_cfg=dict(type='Kaiming', layer='Conv2d')):
        super(Dual_DCA, self).__init__(init_cfg)
        self.embed_dim = out_channels
        self.spatial_size = spatial_size

        self.share_proj_conv = build_conv_layer(conv_cfg, img_feat_channels+pts_feat_channels, out_channels, 1, stride=1, padding=0)
        self.img_proj_conv = build_conv_layer(conv_cfg, img_feat_channels, out_channels, 1, stride=1, padding=0)
    ...
    def forward(self, inputs):
        ...
        share_feats = self.share_proj_conv(torch.cat([img_feats, pts_feats], axis=1)) #(B, C, H, W)
        proj_img_feats = self.img_proj_conv(img_feats) #(B, C, H, W)
        proj_pts_feats = self.pts_proj_conv(pts_feats) #(B, C, H, W)

        B,_,_,_ = pts_feats.size()
        device = share_feats.device

        query = share_feats.permute(0,2,3,1).contiguous().view(B,-1,self.embed_dim).permute(1,0,2) #(hw, B, e_d)
        value_img = proj_img_feats.permute(0,2,3,1).contiguous().view(B,-1,self.embed_dim).permute(1,0,2) #(hw, B, e_d)
        ...
        out = (share_ask_img + share_ask_pts).permute(1,0,2).contiguous().view(B, self.spatial_size[0][0], self.spatial_size[0][1], self.embed_dim) #(B,H,W,C)
        return out.permute(0,3,1,2).contiguous()


@NECKS.register_module()
class Dual_DCAV2(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_cfg=dict(type='BN', eps=1e-3, momentum=0.01),
                 init_cfg=dict(type='Kaiming', layer='Conv2d')):
        super(Dual_DCAV2, self).__init__(init_cfg)
        self.embed_dim = out_channels
        self.spatial_size = spatial_size
        ...
        self.linear_proj = nn.Linear(2*self.embed_dim, self.embed_dim, bias=False)
        self.DSA = MultiScaleDeformableAttention(embed_dims=out_channels, num_levels=1)
    ...
    def forward(self, inputs):
        ...
        out = self.linear_proj(torch.cat([share_ask_img, share_ask_pts], dim=2))#(hw, B, e_d)
        out = self.DSA(query = out,
                       query_pos=pos_embed,
                       reference_points=reference_points,
                       spatial_shapes=torch.Tensor(self.spatial_size).long().to(device),
                       level_start_index=torch.Tensor([0]).long().to(device)
                       )#(hw, B, e_d)
        out = out.permute(1,0,2).contiguous().view(B, self.spatial_size[0][0], self.spatial_size[0][1], self.embed_dim) #(B,H,W,C)
        return out.permute(0,3,1,2).contiguous()



@NECKS.register_module()
class MDA(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_cfg=dict(type='BN', eps=1e-3, momentum=0.01),
                 init_cfg=dict(type='Kaiming', layer='Conv2d')):
        super(MDA, self).__init__(init_cfg)
        self.embed_dim = out_channels
        self.spatial_size = spatial_size
        ...
        self.bn2_2 = nn.BatchNorm1d(out_channels)
        self.MSDA_on_img2 = MultiScaleDeformableAttention(embed_dims=out_channels, num_levels=1)
        self.MSDA_on_pts2 = MultiScaleDeformableAttention(embed_dims=out_channels, num_levels=1)
        '''
        self.bn3_1 = nn.BatchNorm1d(out_channels)
        self.linear3 = nn.Linear(out_channels, out_channels)
        self.bn3_2 = nn.BatchNorm1d(out_channels)
        self.MSDA_on_img3 = MultiScaleDeformableAttention(embed_dims=out_channels, num_levels=1)
        self.MSDA_on_pts3 = MultiScaleDeformableAttention(embed_dims=out_channels, num_levels=1)

        self.bn4_1 = nn.BatchNorm1d(out_channels)
        self.linear4 = nn.Linear(out_channels, out_channels)
        self.bn4_2 = nn.BatchNorm1d(out_channels)
        self.MSDA_on_img4 = MultiScaleDeformableAttention(embed_dims=out_channels, num_levels=1)
        self.MSDA_on_pts4 = MultiScaleDeformableAttention(embed_dims=out_channels, num_levels=1)
        '''

        self.bn5_1 = nn.BatchNorm1d(out_channels)
        self.linear5 = nn.Linear(out_channels, out_channels)
        self.bn5_2 = nn.BatchNorm1d(out_channels)
    ...
    def forward(self, inputs):
        ...
        #stage2
        query = self.bn2_1((share_ask_img + share_ask_pts - query).permute(0,2,1)).permute(0,2,1)
        query = query + self.linear2(query)
        query = self.bn2_2(query.permute(0,2,1)).permute(0,2,1)
        share_ask_img = self.MSDA_on_img2(query=query,
                                         value=value_img,
                                         query_pos=pos_embed,
                                         reference_points=reference_points,
                                         spatial_shapes=torch.Tensor(self.spatial_size).long().to(device),
        ...
        share_ask_pts = self.MSDA_on_pts2(query=query,
                                         value=value_pts,
                                         query_pos=pos_embed,
                                         reference_points=reference_points,
                                         spatial_shapes=torch.Tensor(self.spatial_size).long().to(device),
        ...
        '''
        # stage3
        query = self.bn3_1((share_ask_img + share_ask_pts - query).permute(0, 2, 1)).permute(0, 2, 1)
        query = query + self.linear3(query)
        query = self.bn3_2(query.permute(0, 2, 1)).permute(0, 2, 1)
        share_ask_img = self.MSDA_on_img3(query=query,
                                          value=value_img,
                                          query_pos=pos_embed,
                                          reference_points=reference_points,
                                          spatial_shapes=torch.Tensor(self.spatial_size).long().to(device),
        ...
        share_ask_pts = self.MSDA_on_pts3(query=query,
                                          value=value_pts,
                                          query_pos=pos_embed,
                                          reference_points=reference_points,
                                          spatial_shapes=torch.Tensor(self.spatial_size).long().to(device),
        ...
        # stage4
        query = self.bn4_1((share_ask_img + share_ask_pts - query).permute(0, 2, 1)).permute(0, 2, 1)
        query = query + self.linear4(query)
        query = self.bn4_2(query.permute(0, 2, 1)).permute(0, 2, 1)
        share_ask_img = self.MSDA_on_img4(query=query,
                                          value=value_img,
                                          query_pos=pos_embed,
                                          reference_points=reference_points,
                                          spatial_shapes=torch.Tensor(self.spatial_size).long().to(device),
        ...
        share_ask_pts = self.MSDA_on_pts4(query=query,
                                          value=value_pts,
                                          query_pos=pos_embed,
                                          reference_points=reference_points,
                                          spatial_shapes=torch.Tensor(self.spatial_size).long().to(device),
        ...
        '''
        query = self.bn5_1((share_ask_img + share_ask_pts - query).permute(0, 2, 1)).permute(0, 2, 1)
        query = query + self.linear5(query)
        query = self.bn5_2(query.permute(0, 2, 1)).permute(0, 2, 1)
        out = query.permute(1,0,2).contiguous().view(B, self.spatial_size[0][0], self.spatial_size[0][1], self.embed_dim) #(B,H,W,C)
...
class MDAV2(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_cfg=dict(type='BN', eps=1e-3, momentum=0.01),
                 init_cfg=dict(type='Kaiming', layer='Conv2d')):
        super(MDAV2, self).__init__(init_cfg)
        self.embed_dim = out_channels
        self.spatial_size = spatial_size

        self.share_proj_conv = nn.Sequential(build_conv_layer(conv_cfg, img_feat_channels + pts_feat_channels, out_channels, 1,stride=1, padding=0),
                                             build_norm_layer(norm_cfg, out_channels)[1],
                                             nn.ReLU(inplace=True))
        self.img_proj_conv = nn.Sequential(build_conv_layer(conv_cfg, img_feat_channels, out_channels, 1, stride=1, padding=0),
                                           build_norm_layer(norm_cfg, out_channels)[1],
                                           nn.ReLU(inplace=True))
        self.pts_proj_conv = nn.Sequential(build_conv_layer(conv_cfg, pts_feat_channels, out_channels, 1, stride=1, padding=0),
                                           build_norm_layer(norm_cfg, out_channels)[1],
                                           nn.ReLU(inplace=True))
        ...
        self.bn_out1 = nn.BatchNorm1d(out_channels)
        self.linear_out = nn.Linear(out_channels, out_channels)
        self.bn_out2 = nn.BatchNorm1d(out_channels)
    ...
    def forward(self, inputs):
        ...
        atten_channel = self.act(self.linear(self.lateral(share_feats).squeeze())).unsqueeze(-1).unsqueeze(-1)  # (B,C,1,1)
        atten_spatial = self.act(self.spatial_conv(share_feats))  # (B,1,H,W)
        share_feats = share_feats*atten_channel*atten_spatial
        ...
        query = self.bn_out1((share_ask_img + share_ask_pts - query).permute(0, 2, 1)).permute(0, 2, 1)
        query = query + self.linear_out(query)
        query = self.bn_out2(query.permute(0, 2, 1)).permute(0, 2, 1)
        out = query.permute(1, 0, 2).contiguous().view(B, self.spatial_size[0][0], self.spatial_size[0][1],self.embed_dim)  # (B,H,W,C)
        return out.permute(0, 3, 1, 2).contiguous()
