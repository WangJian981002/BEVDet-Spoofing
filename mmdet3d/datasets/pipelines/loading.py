...
def wavelet_denoise(color_img, wavelet='db1', level=1):
    ...
    for channel in channels:
        ...
        for i in range(1, len(coeffs_H)):
            coeffs_H[i] = tuple(
                pywt.threshold(component, value=np.median(np.abs(component)) * 0.7, mode='soft') for component in
                coeffs_H[i])
...
#测试时，将scene级别海报作用于图像
@PIPELINES.register_module()
class Apply_DisPoster_on_batch(object):
    def __init__(self, poster_dir,spoof_cams=['CAM_FRONT', 'CAM_BACK'],is_bilinear=False, is_blur=False, blur_type='mean'):
        self.poster = torch.load(poster_dir) #bgr 0~1
        #self.poster = self.poster.new_ones(self.poster.size())*0.5
        self.spoof_cams = spoof_cams
    ...
    def __call__(self, results):
        ...
        camcenter_yaw = {'CAM_FRONT': 0, 'CAM_BACK': 180, 'CAM_FRONT_RIGHT': -55, 'CAM_FRONT_LEFT': 55, 'CAM_BACK_RIGHT': -110, 'CAM_BACK_LEFT': 110}
        sample_range = (5, 20)
        default_lwh = (3., 0.3, 2.)
        physical_lh = (3.0, 2.0)
        max_search_num = 50
        ...
        poster_h, poster_l = self.poster.size()[:2]
        delta_l, delta_h = physical_lh[0] / poster_l, physical_lh[1] / poster_h
        for spoofcam in self.spoof_cams:
            cam_i = camname_idx[spoofcam]
            gt_box = results['gt_bboxes_3d'].tensor  # (N,9)
            ...
            for _ in range(max_search_num):
                ...
                an = (2 * np.random.rand() - 1) * 15 + camcenter_angle[spoofcam]
                if an < -180:
                    an = -180
                an = an * np.pi / 180.
                cx = r * np.cos(an)
                cy = r * np.sin(an)
                yaw = (2 * np.random.rand() - 1) * 30 + camcenter_yaw[spoofcam]
            ...
            l, h = physical_lh[0], physical_lh[1]
            poster_corner = torch.Tensor([[l / 2, 0, h],
                                          [l / 2, 0, 0],
                                          [-l / 2, 0, 0],
                                          [-l / 2, 0, h]]).unsqueeze(0)  # (1,4,3)

            poster_corner = rotate_points_along_z(poster_corner, torch.Tensor([fake_box[0, 4]])).squeeze(0)  # (4,3)
            poster_corner[:, :] += fake_3d_box[:, :3]  # (4,3)

            '''求解海报平面方程**************************************************************************************'''
            AA = poster_corner[:3, :3].numpy()
            BB = np.array([-1, -1, -1])
            aa, bb, cc = np.linalg.solve(AA, BB)  ## aa x + bb y + cc z + 1 = 0

            '''求解对应于图像中的四个角点*************************************************'''
            cam2lidar = torch.eye(4)
            cam2lidar[:3, :3] = results['img_inputs'][1][cam_i]
            cam2lidar[:3, 3] = results['img_inputs'][2][cam_i]
            lidar2cam = torch.inverse(cam2lidar)

            cam2img = torch.eye(4)
            cam2img[:3, :3] = results['img_inputs'][3][cam_i]
            lidar2img = cam2img.matmul(lidar2cam)
            ...
            post_rot, post_tran = results['img_inputs'][4][cam_i], results['img_inputs'][5][cam_i]
            ...
            if len(img_inner_points) <= 20: continue #如果在图像上的poster像素点少于200，就不要这个实例了
            ...
            RA = R.matmul(R.new_tensor([aa, bb, cc]).unsqueeze(-1))
            T = lidar2img[:3, 3]

            fz = T[0] * RA[0] + T[1] * RA[1] + T[2] * RA[2] - 1
            fm = img_points_orisize[:, 0] * RA[0] + img_points_orisize[:, 1] * RA[1] + RA[2]
            ...
            lidar_inner_points[:, :] -= fake_3d_box[:, :3]
            ...
            if self.is_bilinear:
                ...
                index_h = torch.clip(((h - lidar_inner_points[:, 2]) / h) * 2 - 1, min=-1, max=1)
                grid = torch.cat([index_l.unsqueeze(-1), index_h.unsqueeze(-1)], dim=1).unsqueeze(0).unsqueeze(0)  # (1,1,Nin,2)
                selected_color = torch.nn.functional.grid_sample(self.poster.permute(2, 0, 1).unsqueeze(0),
                                                                 grid, mode='bilinear',
                                                                 align_corners=True)  # (1,3,1,Nin)
                ...
                index_h = torch.clip((h - lidar_inner_points[:, 2]) // delta_h, min=0, max=poster_h - 1).long()
                selected_color = self.poster[index_h, index_l, :]  # (Nin, 3) bgr 0~1 gpu
            ...
            selected_color = (selected_color - torch.from_numpy(mean)) / torch.from_numpy(std)  # (Nin, 3) 归一化
            imgs[cam_i, :, img_inner_points[:, 1],img_inner_points[:, 0]] = selected_color.T
        if self.is_blur:
            blur_imgs = []
            for i in range(6):
                img = imgs[i]
                img = mmlabDeNormalize(img)  # rgb (H,W,3)
                if self.blur_type is 'mean':
                    img = cv2.blur(img, (3, 3))
                elif self.blur_type is 'gaussian':
                    img = cv2.GaussianBlur(img, (3, 3), 2)
                elif self.blur_type is 'NlMean':
                    img = cv2.fastNlMeansDenoisingColored(img.astype(np.uint8), None, 10, 10, 10, 7)
                elif self.blur_type is 'wavelet':
                    img = wavelet_denoise(img, wavelet='db2', level=1)
                elif self.blur_type is 'jpegcomperss':
                    img_encode = cv2.imencode('.jpeg', img, [cv2.IMWRITE_JPEG_QUALITY, 50])[1]
                    img = cv2.imdecode(img_encode, cv2.IMREAD_COLOR)
                else:
                    assert False

                img = mmlabNormalize(img)  # bgr 标准化
                blur_imgs.append(img)
            blur_imgs = torch.stack(blur_imgs)
            imgs = blur_imgs
...
#训练时，将instance hiding海报作用于图像
@PIPELINES.register_module()
class Apply_insHiding_during_training(object):
    def __init__(self, poster_dir, with_patch_mask = False):
        self.leaning_poster_side = torch.load(os.path.join(poster_dir,'poster_8.pth'))
        self.leaning_poster_front = torch.load(os.path.join(poster_dir,'front_poster_8.pth'))
        self.leaning_poster_back = torch.load(os.path.join(poster_dir,'back_poster_8.pth'))
        self.with_patch_mask = with_patch_mask
    ...
    def __call__(self, results):
        ...
        camcenter_angle = {'CAM_FRONT_LEFT': 145, 'CAM_FRONT': 90, 'CAM_FRONT_RIGHT': 35, 'CAM_BACK_LEFT': -160,'CAM_BACK': -90, 'CAM_BACK_RIGHT': -20}
        physical_lh = (3.8, 0.4)
        delta_angle = 30  # 每个相机中心角度左右范围FOV限制
        max_put_dist = 20  # 待贴poster的车辆最远距离
        poster_h, poster_l = self.leaning_poster_side.size()[:2]
        delta_l, delta_h = physical_lh[0] / poster_l, physical_lh[1] / poster_h
        # 前后poster物理尺寸和分辨率完全一致
        fb_physical_lh = (1.2, 0.4)
        fb_poster_h, fb_poster_l = self.leaning_poster_front.size()[:2]
        fb_delta_l, fb_delta_h = fb_physical_lh[0] / fb_poster_l, fb_physical_lh[1] / fb_poster_h
        fb_rela_factor = 1. / (0.3333)

        imgs, rots, trans, intrins, post_rots, post_trans, bda_rot = results['img_inputs']
        if self.with_patch_mask:
            patch_mask = torch.zeros(imgs.size()) #(N,3,H,W)
            results['patch_mask'] = patch_mask


        gt_box = results['gt_bboxes_3d'].tensor  # (N,9)
        if len(gt_box) == 0: return results
        gt_car = gt_box[results['gt_labels_3d'] == 0]  # (M,9)
        if len(gt_car) == 0: return results
        for cam_i, cam in enumerate(cam_names):
            inner_points_num = []

            cam_center = camcenter_angle[cam]
            left_bound, right_bound = cam_center - delta_angle, cam_center + delta_angle
            if cam == 'CAM_BACK_LEFT': left_bound = -180  # 防止BL相机角度越界情况
            gt_car_angle = torch.atan2(gt_car[:, 1], gt_car[:, 0]) * 180 / torch.pi
            a_mask = (gt_car_angle > left_bound) & (gt_car_angle < right_bound)
            gt_car_in_range = gt_car[a_mask]  # (n,9)
            if len(gt_car_in_range) == 0:
                continue  # 视角FOV筛选
            ego_dist = torch.sqrt(gt_car_in_range[:, 0] ** 2 + gt_car_in_range[:, 1] ** 2)
            selected_gt = gt_car_in_range[torch.argmin(ego_dist)]  # (9) #选取FOV视角内最近的车辆
            if ego_dist[torch.argmin(ego_dist)] > max_put_dist:
                continue  # 车辆距离筛选
            side_center = torch.Tensor([[0, -selected_gt[4] / 2., selected_gt[5] / 2.],
                                        [0, selected_gt[4] / 2., selected_gt[5] / 2.]]).unsqueeze(0)  # (1,2,3)

            side_center = rotate_points_along_z(side_center, torch.Tensor([selected_gt[6]])).squeeze(0)  # (2,3)
            side_center[:, :] += selected_gt[:3].unsqueeze(0)  # (2,3) 右侧面、左侧面中心点坐标

            side_center_dist = side_center[:, 0] ** 2 + side_center[:, 1] ** 2  # 2 [右，左]
            if side_center_dist[0] < side_center_dist[1]:  # 贴在右侧面
                poster_corner = torch.Tensor(
                    [[-physical_lh[0] / 2., 0 - selected_gt[4] / 2., physical_lh[1] / 2. + selected_gt[5] / 2.],
                     [physical_lh[0] / 2., 0 - selected_gt[4] / 2., physical_lh[1] / 2. + selected_gt[5] / 2.],
                     [physical_lh[0] / 2., 0 - selected_gt[4] / 2., -physical_lh[1] / 2. + selected_gt[5] / 2.],
                     [-physical_lh[0] / 2., 0 - selected_gt[4] / 2.,
                      -physical_lh[1] / 2. + selected_gt[5] / 2.]]).unsqueeze(0)
                poster_corner = rotate_points_along_z(poster_corner, torch.Tensor([selected_gt[6]])).squeeze(0)  # (4,3)
                poster_corner[:, :] += selected_gt[:3].unsqueeze(0)  # (4,3) 右侧面海报四个角点坐标
            else:
                poster_corner = torch.Tensor(
                    [[physical_lh[0] / 2., 0 + selected_gt[4] / 2., physical_lh[1] / 2. + selected_gt[5] / 2.],
                     [-physical_lh[0] / 2., 0 + selected_gt[4] / 2., physical_lh[1] / 2. + selected_gt[5] / 2.],
                     ...
                     [physical_lh[0] / 2., 0 + selected_gt[4] / 2.,
                      -physical_lh[1] / 2. + selected_gt[5] / 2.]]).unsqueeze(0)
                poster_corner = rotate_points_along_z(poster_corner, torch.Tensor([selected_gt[6]])).squeeze(0)  # (4,3)
                poster_corner[:, :] += selected_gt[:3].unsqueeze(0)  # (4,3) 左侧面海报四个角点坐标
            '''求解海报平面方程'''
            AA = poster_corner[:3, :3].numpy()
            BB = np.array([-1, -1, -1])
            aa, bb, cc = np.linalg.solve(AA, BB)  ## aa x + bb y + cc z + 1 = 0
            ...
            path_points = points[np.where(mask)]  # (Nin,2) [x,y]
            img_inner_points = torch.from_numpy(path_points)  # (Nin,2) [x,y]
            inner_points_num.append(len(img_inner_points))
            if len(img_inner_points) > 100:
                '''将2D区域内所有像素点project到3D LiDAR系下********************************************************************'''
                img_points_orisize = (img_inner_points - post_tran[:2].unsqueeze(0)).matmul(
                    torch.inverse(post_rot.T[:2, :2]))  # (Nin,2)
                R = torch.inverse(lidar2img[:3, :3].T)
                ...
                lidar_inner_points = (img_points_orisize_C - T.unsqueeze(0)).matmul(R)  # (Nin, 3)
                '''找到每个3D点在poster上的颜色索引,并put到原图上'''
                lidar_inner_points[:, :] -= selected_gt[:3].unsqueeze(0)
                lidar_inner_points = rotate_points_along_z(lidar_inner_points.unsqueeze(0),
                                                           -1 * torch.Tensor([selected_gt[6]])).squeeze(0)  # (Nin,3)
                if side_center_dist[0] < side_center_dist[1]:  # 右侧面
                    lidar_inner_points[:, 1] += selected_gt[4] / 2.
                    lidar_inner_points[:, 2] -= selected_gt[5] / 2. - physical_lh[1] / 2.
                    lidar_inner_points[:, 0] += physical_lh[0] / 2.
                else:
                    lidar_inner_points[:, 1] -= selected_gt[4] / 2.
                    lidar_inner_points[:, 2] -= selected_gt[5] / 2. - physical_lh[1] / 2.
                    lidar_inner_points[:, 0] += physical_lh[0] / 2.
                index_l = torch.clip(lidar_inner_points[:, 0] // delta_l, min=0, max=poster_l - 1).long()
                index_h = torch.clip((physical_lh[1] - lidar_inner_points[:, 2]) // delta_h, min=0,max=poster_h - 1).long()
                selected_color = self.leaning_poster_side[index_h, index_l, :]  # (Nin, 3) bgr 0~1 gpu
                contrast = round(random.uniform(0.7, 1.0), 10)
                brightness = round(random.uniform(-0.3, 0.2), 10)
                selected_color = selected_color * contrast + brightness
                selected_color[selected_color > 1] = 1
                ...
                imgs[cam_i, :, img_inner_points[:, 1],img_inner_points[:, 0]] = selected_color.T
                if self.with_patch_mask:
                    patch_mask[cam_i, :, img_inner_points[:, 1],img_inner_points[:, 0]] = 1
            #处理其余相机
            for ccam_i, ccam in enumerate(cam_names):
                if ccam == cam: continue
                # 求解对应于图像中的四个角点*************************************************'''
                cam2lidar = torch.eye(4)
                cam2lidar[:3, :3] = results['img_inputs'][1][ccam_i]
                cam2lidar[:3, 3] = results['img_inputs'][2][ccam_i]
                lidar2cam = torch.inverse(cam2lidar)
                cam2img = torch.eye(4)
                cam2img[:3, :3] = results['img_inputs'][3][ccam_i]
                lidar2img = cam2img.matmul(lidar2cam)
                img_corner = poster_corner.matmul(lidar2img[:3, :3].T) + lidar2img[:3, 3].unsqueeze(0)
                img_corner = torch.cat([img_corner[:, :2] / img_corner[:, 2:3], img_corner[:, 2:3]], 1)
                post_rot, post_tran =results['img_inputs'][4][ccam_i], results['img_inputs'][5][ccam_i]
                img_corner = img_corner.matmul(post_rot.T) + post_tran.unsqueeze(0)
                ...
                if side_center_dist[0] < side_center_dist[1]:  # 右侧面
                    ...
                    lidar_inner_points[:, 0] += physical_lh[0] / 2.
                index_l = torch.clip(lidar_inner_points[:, 0] // delta_l, min=0, max=poster_l - 1).long()
                index_h = torch.clip((physical_lh[1] - lidar_inner_points[:, 2]) // delta_h, min=0,
                                     max=poster_h - 1).long()
                selected_color = self.leaning_poster_side[index_h, index_l, :]  # (Nin, 3) bgr 0~1 gpu
                contrast = round(random.uniform(0.7, 1.0), 10)
                brightness = round(random.uniform(-0.3, 0.2), 10)
                ...
                selected_color[selected_color < 0] = 0
                selected_color = (selected_color - torch.from_numpy(mean)) / torch.from_numpy(std)# (Nin, 3) 归一化
                imgs[ccam_i, :, img_inner_points[:, 1], img_inner_points[:, 0]] = selected_color.T
                if self.with_patch_mask:
                    patch_mask[ccam_i, :, img_inner_points[:, 1], img_inner_points[:, 0]] = 1

            '''重复上述过程，对车辆前或后面进行海报张贴*********************************************************************************************************'''
            fb_center = torch.Tensor([[selected_gt[3] / 2., 0, selected_gt[5] / 2.],
                                      [-selected_gt[3] / 2., 0, selected_gt[5] / 2.]]).unsqueeze(0)  # (1,2,3)

            fb_center = rotate_points_along_z(fb_center, torch.Tensor([selected_gt[6]])).squeeze(0)  # (2,3)
            fb_center[:, :] += selected_gt[:3].unsqueeze(0)  # (2,3) 前面、后面中心点坐标

            fb_center_dist = fb_center[:, 0] ** 2 + fb_center[:, 1] ** 2  # 2 [前，后]
            if fb_center_dist[0] < fb_center_dist[1]:  # 贴在前面
                poster_corner = torch.Tensor([
                    [0 + selected_gt[3] / 2., -fb_physical_lh[0] / 2.,
                     fb_physical_lh[1] / 2. + selected_gt[5] / fb_rela_factor],
                    [0 + selected_gt[3] / 2., fb_physical_lh[0] / 2.,
                     fb_physical_lh[1] / 2. + selected_gt[5] / fb_rela_factor],
                    [0 + selected_gt[3] / 2., fb_physical_lh[0] / 2.,
                     -fb_physical_lh[1] / 2. + selected_gt[5] / fb_rela_factor],
                ...
                poster_corner = rotate_points_along_z(poster_corner, torch.Tensor([selected_gt[6]])).squeeze(0)  # (4,3)
                poster_corner[:, :] += selected_gt[:3].unsqueeze(0)  # (4,3) 前面海报四个角点坐标
            else:
                poster_corner = torch.Tensor([
                    [0 - selected_gt[3] / 2., fb_physical_lh[0] / 2.,
                     fb_physical_lh[1] / 2. + selected_gt[5] / fb_rela_factor],
            ...
            path_points = points[np.where(mask)]  # (Nin,2) [x,y]
            img_inner_points = torch.from_numpy(path_points)  # (Nin,2) [x,y]
            if inner_points_num[0] + len(img_inner_points) < 300:
                continue  # 筛出掉图像海报点数过少的
            if len(img_inner_points) > 50:
                #将2D区域内所有像素点project到3D LiDAR系下
                ...
                lidar_inner_points = rotate_points_along_z(lidar_inner_points.unsqueeze(0),
                                                           -1 * torch.Tensor([selected_gt[6]])).squeeze(0)  # (Nin,3)
                if fb_center_dist[0] < fb_center_dist[1]:  # 前面
                    lidar_inner_points[:, 0] -= selected_gt[3]/2.
                    lidar_inner_points[:, 1] += fb_physical_lh[0]/2.
                    lidar_inner_points[:, 2] -= -fb_physical_lh[1]/2.+ selected_gt[5]/fb_rela_factor
                else:
                    lidar_inner_points[:, 0] += selected_gt[3]/2.
                    lidar_inner_points[:, 1] += fb_physical_lh[0]/2.
                    lidar_inner_points[:, 2] -= -fb_physical_lh[1]/2.+ selected_gt[5]/fb_rela_factor

                index_l = torch.clip(lidar_inner_points[:, 1] // fb_delta_l, min=0, max=fb_poster_l - 1).long()
                index_h = torch.clip((fb_physical_lh[1] - lidar_inner_points[:, 2]) // fb_delta_h, min=0,max=fb_poster_h - 1).long()
                if fb_center_dist[0] < fb_center_dist[1]:
                    selected_color = self.leaning_poster_front[index_h, index_l, :]  #(Nin, 3) bgr 0~1 gpu
                else:
                    selected_color = self.leaning_poster_back[index_h, index_l, :]  # (Nin, 3) bgr 0~1 gpu
                contrast = round(random.uniform(0.7, 1.0), 10)
                brightness = round(random.uniform(-0.3, 0.2), 10)
                selected_color = selected_color * contrast + brightness
                selected_color[selected_color > 1] = 1
                selected_color[selected_color < 0] = 0
                selected_color = (selected_color - torch.from_numpy(mean)) / torch.from_numpy(std)
                imgs[cam_i, :, img_inner_points[:, 1],img_inner_points[:, 0]] = selected_color.T
                if self.with_patch_mask:
                    patch_mask[cam_i, :, img_inner_points[:, 1],img_inner_points[:, 0]] = 1
            for ccam_i, ccam in enumerate(cam_names):
                ...
                index_l = torch.clip(lidar_inner_points[:, 1] // fb_delta_l, min=0, max=fb_poster_l - 1).long()
                index_h = torch.clip((fb_physical_lh[1] - lidar_inner_points[:, 2]) // fb_delta_h, min=0,
                                     max=fb_poster_h - 1).long()
                if fb_center_dist[0] < fb_center_dist[1]:
                    selected_color = self.leaning_poster_front[index_h, index_l, :]  # (Nin, 3) bgr 0~1 gpu
                else:
                    selected_color = self.leaning_poster_back[index_h, index_l, :]  # (Nin, 3) bgr 0~1 gpu
...
#训练时，将scene级别海报作用于图像
@PIPELINES.register_module()
class Apply_sceneCreating_during_training(object):
    def __init__(self, poster_dir,spoof_cams_number,is_bilinear=False,with_patch_mask = False):
        self.poster = torch.load(poster_dir) #bgr 0~1
        #self.poster = self.poster.new_ones(self.poster.size())*0.5
        self.spoof_cams_number = spoof_cams_number
        self.is_bilinear = is_bilinear
        self.with_patch_mask = with_patch_mask
    ...
    def __call__(self, results):
        ...
        sps = np.random.choice(['CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_BACK_LEFT', 'CAM_BACK', 'CAM_BACK_RIGHT'], self.spoof_cams_number, replace=False)
        for spoofcam in sps:
            cam_i = camname_idx[spoofcam]
            gt_box = results['gt_bboxes_3d'].tensor  # (N,9)
            gt_bev = gt_box[:, [0, 1, 3, 4, 6]]  # (N,5) [cx,cy,h,w,theta]
...
#训练时，将spoofing海报作用于图像
@PIPELINES.register_module()
class Apply_Spoofing(object):
    def __init__(self, poster_dir,spoof_cams=['CAM_FRONT', 'CAM_BACK'],is_bilinear=True, with_patch_mask = False):
        self.poster = torch.load(poster_dir) #bgr 0~1
        #self.poster = self.poster.new_ones(self.poster.size())*0.5
        self.spoof_cams = spoof_cams
        self.is_bilinear = is_bilinear
    ...
    def __call__(self, results):
        ...
        poster_w, poster_l = self.poster.size()[:2]
        ...
        for spoofcam in self.spoof_cams:
            ...
            if self.is_bilinear:
                ...
                selected_color = torch.nn.functional.grid_sample(self.poster.permute(2, 0, 1).unsqueeze(0),grid, mode='bilinear',align_corners=True)  # (1,3,1,Nin)
                selected_color = selected_color.squeeze().permute(1, 0)

            else:
                index_l = torch.clip(lidar_inner_points[:, 0] // delta_l, min=0, max=poster_l - 1).long()
                index_w = torch.clip((w - lidar_inner_points[:, 1]) // delta_w, min=0, max=poster_w - 1).long()
                selected_color = self.poster[index_w, index_l, :]  # (Nin, 3) bgr 0~1 gpu
...
@PIPELINES.register_module()
class Apply_Spoofing4D(object):
    def __init__(self, poster_dir,spoof_cams=['CAM_FRONT', 'CAM_BACK'],is_bilinear=True):
        self.poster = torch.load(poster_dir) #bgr 0~1
        #self.poster = self.poster.new_ones(self.poster.size())*0.5
        self.spoof_cams = spoof_cams
        self.is_bilinear = is_bilinear
    ...
    def __call__(self, results):
        ...
        for spoofcam in self.spoof_cams:
            ...
            for time_i in range(num_adj+1):
                '''求解对应于图像中的四个角点,暂未约束超出图像边界的情况*************************************************'''
                cam2lidar = torch.eye(4)
                cam2lidar[:3, :3] = results['img_inputs'][1][6 * time_i + cam_i]
                cam2lidar[:3, 3] = results['img_inputs'][2][6 * time_i + cam_i]
                lidar2cam = torch.inverse(cam2lidar)

                cam2img = torch.eye(4)
                cam2img[:3, :3] = results['img_inputs'][3][6 * time_i + cam_i]
                lidar2img = cam2img.matmul(lidar2cam)
                img_corner = poster_corner.matmul(lidar2img[:3, :3].T) + lidar2img[:3, 3].unsqueeze(0)
                if (img_corner[:, 2] < 1).sum(): continue
                img_corner = torch.cat([img_corner[:, :2] / img_corner[:, 2:3], img_corner[:, 2:3]], 1)
                post_rot, post_tran = results['img_inputs'][4][6 * time_i + cam_i], \
                                      results['img_inputs'][5][6 * time_i + cam_i]
                img_corner = img_corner.matmul(post_rot.T) + post_tran.unsqueeze(0)
                img_corner = img_corner[:, :2]  # (4,2)
                ...
                selected_color = (selected_color - torch.from_numpy(mean)) / torch.from_numpy(std)  # (Nin, 3) 归一化
                imgs[(num_adj+1)*cam_i+time_i, :, img_inner_points[:,1], img_inner_points[:,0]] = selected_color.T
...
@PIPELINES.register_module()
class Apply_GAN_Spoofing(object):
    def __init__(self, poster_dir,spoof_cams=['CAM_FRONT', 'CAM_BACK'],is_bilinear=True, with_patch_mask = False):
        self.poster = torch.load(poster_dir) #bgr 0~1 (N,3,H,W)
        #self.poster = self.poster.new_ones(self.poster.size())*0.5
        self.spoof_cams = spoof_cams
        self.is_bilinear = is_bilinear
        self.with_patch_mask = with_patch_mask
    ...
    def __call__(self, results):
        ...
        use_poster = copy.deepcopy(self.poster[np.random.randint(0,self.poster.size(0))])
        ...
        for spoofcam in self.spoof_cams:
            ...
            if self.is_bilinear:
                ...
                index_w = torch.clip(((w - lidar_inner_points[:, 1]) / w) * 2 - 1, min=-1, max=1)
                grid = torch.cat([index_l.unsqueeze(-1), index_w.unsqueeze(-1)], dim=1).unsqueeze(0).unsqueeze(0)  # (1,1,Nin,2)
                selected_color = torch.nn.functional.grid_sample(use_poster.unsqueeze(0),grid, mode='bilinear',align_corners=True)  # (1,3,1,Nin)
                selected_color = selected_color.squeeze().permute(1, 0)

            else:
                index_l = torch.clip(lidar_inner_points[:, 0] // delta_l, min=0, max=poster_l - 1).long()
                index_w = torch.clip((w - lidar_inner_points[:, 1]) // delta_w, min=0, max=poster_w - 1).long()
                selected_color = use_poster[:, index_w, index_l].T  # (Nin, 3) bgr 0~1 gpu
...
'''XinGangDet数据集相关'''
@PIPELINES.register_module()
class XGD_PrepareImageInputs(object):
    def __init__(
        self,
        data_config,
        is_train=False
        ...
        R1 = np.array([[-3.5916531365591808e-03, 6.0928467351954452e-03, 9.9997498831041176e-01, 1.0832665893681656e-01],
             [-9.9996540136473944e-01, -7.5248648249737204e-03, -3.5457697078721262e-03, -6.6723962782960699e-03],
             [7.5030727840023070e-03, -9.9995312571541428e-01, 6.1196626353643424e-03, -1.0314691835591448e-01],
             [0., 0., 0., 1.]],np.float32)
        camera_matrix = np.array([[1.0048436619658388e+03, 0., 1.0319672021852414e+03],
                                  [0., 1.0079631818777184e+03, 5.6168660715578733e+02],
                                  [0., 0., 1.]],np.float32)
        R = np.linalg.inv(R1)

        self.lidar2cam = torch.from_numpy(R)
        self.intrins = torch.from_numpy(camera_matrix)
        self.cam2lidar = torch.inverse(self.lidar2cam)
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        filename = results['img_dir']
        img = Image.open(filename)

        rot = self.cam2lidar[:3,:3]
        tran = self.cam2lidar[:3,3]
        intrin = self.intrins
        ...
        resize, resize_dims, crop, flip, rotate = img_augs
        img, post_rot2, post_tran2 = self.img_transform(img, post_rot,
                               post_tran,
                               resize=resize,
                               resize_dims=resize_dims,
                               crop=crop,
        ...
        imgs = self.normalize_img(img).unsqueeze(0) #(1,3,H,W)
        rots = rot.unsqueeze(0) #(1,3,3)
        trans = tran.unsqueeze(0) #(1,3)
        intrins = intrin.unsqueeze(0) #(1,3,3)
        post_rots = post_rot.unsqueeze(0) #(1,3,3)
        post_trans = post_tran.unsqueeze(0) #(1,3)
...
@PIPELINES.register_module()
class XGD_LoadAnnotationsBEVDepth(object):
    ...
    def __call__(self, results):
        if 'gt_bboxes_3d' not in results.keys():
            imgs, rots, trans, intrins = results['img_inputs'][:4]
            post_rots, post_trans = results['img_inputs'][4:]
        ...
        if len(gt_boxes) == 0:
            gt_boxes = torch.zeros(0, 9)
        results['gt_bboxes_3d'] = LiDARInstance3DBoxes(torch.cat([gt_boxes,torch.zeros(gt_boxes.size(0),2)],dim=1), box_dim=9,origin=(0.5, 0.5, 0.5))
        results['gt_labels_3d'] = gt_labels
        imgs, rots, trans, intrins = results['img_inputs'][:4]
        post_rots, post_trans = results['img_inputs'][4:]
        results['img_inputs'] = (imgs, rots, trans, intrins, post_rots,
...
'''KITTI数据集相关'''
@PIPELINES.register_module()
class KITTI_PrepareImageInputs(object):
    def __init__(
        self,
        data_config,
        is_train=False
